...
class GraphBase {
 public:
  /// Format the input for future computing, which is much faster for loading. Vector serialization
  /// is used.
  static void FormatGraph(const std::string filename, ProbDist probDist, const float sum,
                          const float prob, const std::string skewType) {
    size_t numV, numE;
    ...
    numV = maxVal + 1;
    numE = lineCount;
    log_info("nodes: %ld, edges: %ld", numV, numE);
